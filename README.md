...
- `src/<Source>/package.json`: useful, but not what the current bundler uses to discover entrypoints. The toolchain currently discovers live sources from `src/<Folder>/<Folder>.ts`. These local manifests are still worth keeping because they make each source self-describing and, with `pnpm-workspace.yaml`, also make those folders valid workspace packages.
- `src/<Source>/README.md`: optional maintainer documentation only.
- `templates/*`: starter code and docs. Templates are not compiled by the root build until you copy one into `src/` and rename the files to match the new source folder.

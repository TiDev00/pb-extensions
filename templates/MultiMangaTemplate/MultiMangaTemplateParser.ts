...
export class MultiMangaTemplateParser {
  ...
  parseMangaDetails($: CheerioAPI, mangaId: string): SourceManga {
    ...
    const description =
      ...
      $("div.description-summary").text().trim() ||
      "";
    ...
    // ── Artist ────────────────────────────────────────────────────────────
    const artist = $("div.artist-content a").first().text().trim() || author;
    ...
    if (statusRaw.includes("hiatus")) status = "Hiatus";
  ...
  hasNextPage($: CheerioAPI): boolean {
    return (
      $('a.next, a[rel="next"], div.nav-links a.next, .navigation-ajax a.btn')
        .length > 0
    );
  }
  ...
  private parseTileList($: CheerioAPI, selector: string): PartialSourceManga[] {
    ...
    $(selector).each((_i, el) => {
      ...
      const image =
        $("img", el).attr("data-src") ?? $("img", el).attr("src") ?? "";

      const title =
        $("img", el).attr("alt")?.trim() ||
        $("div.post-title", el).text().trim() ||
        anchor.text().trim() ||
        mangaId;

      const subtitle = $("span.chapter, span.latest-chapter, .chapter-name", el)
        .text()
        .trim();
